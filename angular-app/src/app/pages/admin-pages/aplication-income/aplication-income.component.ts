...
import {
  ...
  yearLabels,
  initialData,
} from 'src/app/model/reservationCount';
import { AplicationIncomeService } from 'src/app/service/aplication-income.service';

@Component({
  selector: 'app-aplication-income',
  templateUrl: './aplication-income.component.html',
  styleUrls: ['./aplication-income.component.css'],
})
export class AplicationIncomeComponent implements OnInit {
  incomeChart!: Chart;
  ...
  constructor(private aplicationIncomeService: AplicationIncomeService) {}
  ...
  addDaysLabels(): string[] {
    ...
    for (startYear; startYear <= endYear; startYear++) {
      for (let i = 0; i <= 11; i++) {
        for (let j = 1; j <= 31; j++) {
          labelsDays.push(
            String(j) + ' ' + this.months[i] + ' ' + String(startYear)
          );
        }
      }
    }
  ...
  incomeChartYearly() {
    ...
    this.aplicationIncomeService
      .getIncomeYearly(this.duration)
      .subscribe((data) => {
  ...
  incomeChartMonthly() {
    ...
    this.incomeChart.config.type = 'pie' as ChartType;
    this.aplicationIncomeService
      .getIncomeMonthly(this.duration)
      .subscribe((data) => {
        let arr = [];
  ...
  incomeChartDaily() {
    ...
    this.incomeChart.config.type = 'pie' as ChartType;
    this.aplicationIncomeService
      .getIncomeWeeklyDaily(this.duration)
      .subscribe((data) => {
        let arr = [];
        for (let table of data.dailySum)
          for (let row of table) for (let e of row) arr.push(e);
        this.incomeChart.data.datasets[0].data = arr;
        this.incomeChart.update();
      });
    this.incomeChart.update();
